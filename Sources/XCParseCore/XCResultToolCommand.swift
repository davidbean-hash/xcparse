...
open class XCResultToolCommand {
    ...
    open class Export: XCResultToolCommand {
        ...
        public init(withXCResult xcresult: XCResult, attachment: ActionTestAttachment, outputPath: String, overrideFilename: String? = nil) {
            if let identifier = attachment.payloadRef?.id {
                self.id = identifier;

                // Now let's figure out the filename & path
                let filename = overrideFilename ?? attachment.filename ?? identifier
